...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prepared representative cases on ZXH CUDA backend.")
    ...
    parser.add_argument("--eager-expand-all", action="store_true")
...
def _run_iteration(*, sim, circuit, shots: int, qasm_mod) -> dict[str, Any]:
    sim.clear_gates()
...
def main() -> int:
    ...
    run_id = args.run_id or f"zxh_cuda_{timestamp()}"
    ...
    metadata_obj: dict[str, Any] = {
        ...
        "selected_backends": ["zxh-cuda"],
        "selected_families": [row["family"] for row in rows],
        "warmup": args.warmup,
        "repeats": args.repeats,
        "shots": args.shots,
        "time_budget_s": args.time_budget_s,
        "disable_x": args.disable_x,
        "eager_expand_all": args.eager_expand_all,
        ...
        "zxh_cuda_stage_dir": repo_rel(stage_dir),
    }
    ...
    try:
        for case_row in rows:
            ...
            print(f"[run] family={family} backend=zxh-cuda warmup={args.warmup} repeats={args.repeats}", flush=True)

            try:
                sim = zxhsim_mod.ZXH(
                    execution_circuit.num_qubits,
                    disable_x=args.disable_x,
                    eager_expand_all=args.eager_expand_all,
                )
                ...
                raw_rows.append(row)
                errors.append(row)
                print(f"[timeout] family={family} backend=zxh-cuda error={row['error']}", flush=True)
                ...
                raw_rows.append(row)
                errors.append(row)
                print(f"[error] family={family} backend=zxh-cuda error={row['error']}", flush=True)
    finally:
        zxhsim_mod.finalize()
