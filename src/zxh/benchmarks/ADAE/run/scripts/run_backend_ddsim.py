...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run prepared representative cases on DDSIM backend.")
...
def _run_iteration(*, circuit, shots: int, ddsim_backend) -> dict[str, Any]:
...
def main() -> int:
    ...
    run_id = args.run_id or f"ddsim_{timestamp()}"
    ...
    metadata_obj: dict[str, Any] = {
        ...
        "mqt_ddsim_version": metadata.version("mqt.ddsim"),
    }
    ...
    for case_row in rows:
        ...
        print(f"[run] family={family} backend=ddsim warmup={args.warmup} repeats={args.repeats}", flush=True)

        try:
            for warmup_index in range(args.warmup):
                backend_metrics = _run_iteration(circuit=canonical_circuit, shots=args.shots, ddsim_backend=ddsim_backend)
                enforce_per_run_time_budget(
                    family=family,
                    backend="ddsim",
                    phase="warmup",
            ...
            raw_rows.append(row)
            errors.append(row)
            print(f"[timeout] family={family} backend=ddsim error={row['error']}", flush=True)
            ...
            raw_rows.append(row)
            errors.append(row)
            print(f"[error] family={family} backend=ddsim error={row['error']}", flush=True)
