...
void apply_diag_window_omp(val_t *ptr, size_t n)
{
    const long long n64 = static_cast<long long>(n);
#pragma omp parallel for schedule(static)
    for (long long ii = 0; ii < n64; ii++)
    {
        const raddr_t xr = static_cast<raddr_t>(static_cast<size_t>(ii));
        ...
        if (theta_sum != 0.0)
            ptr[static_cast<size_t>(ii)] *= Phase(theta_sum);
    }
}
...
#pragma omp parallel for schedule(static)
    for (long long ii = 0; ii < n64; ii++)
    {
        const size_t off = static_cast<size_t>(ii);
        if (S_q_block.eval(off))
            continue;
        const size_t peer = off ^ delta;
        if (peer >= n)
            continue;
        const val_t tmp = ptr[off];
        ptr[off] = ptr[peer];
        ptr[peer] = tmp;
    }
}
